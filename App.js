...
class App extends React.Component {
  ...
  async loadProductList() {
    try {
      let productListJson = await AsyncStorage.getItem('productList');
      ...
      console.log('Exception loadFrom db' + exception);
    }
  }

  async persistProductList(productList_) {
    try {
      await AsyncStorage.setItem('productList', JSON.stringify(productList_))
        .then(() => {
          this.setState({ productList: productList_ });
        })
        .catch(() => {
          console.error('Error when save productList');
        });
    } catch (exception) {
      console.log('Exception persistProductList :' + exception);
    }
  }
  ...
  updateProductStatus(product_, status_) {
    let productList = this.state.productList;
    for (let i in productList) {
      if (productList[i].id == product_.id) {
        productList[i].status = status_;
      }
    }
    this.persistProductList(productList).then(
  ...
  addProduct() {
    ...
    if (productToAdd.texte.length == 0) {
      return;
    }

    productToAdd.id = productList.length;
    productToAdd.status = STATUS_PENDING;
        ...
        <SafeAreaView>
          <ScrollView >
            <View style={styles.sectionContainer}>
                      ...
                      product.status !== STATUS_PENDING ?
                        ...
                        <View key={product.id} flexDirection="row">
                          ...
                          <Switch
                            ...
                            onValueChange={() => this.updateProductStatus(product, STATUS_ADDED)}
                            value={false}
                          />
                        </View>
                    )
              ...
              <Text style={styles.sectionTitle}>Déjà ajoutée au caddie</Text>
                      ...
                      product.status !== STATUS_ADDED ?
                        ...
                        <View key={product.id} flexDirection="row">
                          ...
                          <Switch
                            trackColor={{ false: "#767577", true: "#81b0ff" }}
                            ios_backgroundColor="#3e3e3e"
                            onValueChange={() => this.updateProductStatus(product, STATUS_PENDING)}
                            value={true}
                          />
                        </View>
                    )
